...
public class MyHashMap<K, V> {
    ...
    private DoublyCircularLinkedList<MyEntry<K, V>>[] index;
    ...
    public DoublyCircularLinkedList<MyEntry<K, V>> get(int index) {
    ...
    public V get(K key) {
        ...
        DoublyCircularLinkedList<MyEntry<K, V>> currentList = this.index[hashKey];
...
//            this.size++;
    ...
    private void rearrangeSet() {
        this.indexSize *= 2;
        DoublyCircularLinkedList<MyEntry<K, V>>[] newIndex = new DoublyCircularLinkedList[this.indexSize];
        initArrayLists(newIndex);

        for (DoublyCircularLinkedList<MyEntry<K, V>> currentList : this.index) {
            for (int i = 0; i < currentList.size(); ++i) {
                MyEntry<K, V> currentItem = currentList.get(i);
